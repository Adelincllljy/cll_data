...
module FixSql
  ...
  def self.fix_virtual_file(user,repo)
    puts "build"
    builds=[]
    parent_dir = File.join('build_logs/',  "#{user}@#{repo}")
    File.open(File.join(parent_dir,"build_state_temp.json"), "r") do |file|
      file.each_line do |line|
          #puts line()
          #ch=line
          builds << JSON.parse(line)
          #puts ch
          #break
      end
    end
  ...
  def self.insert_into_temp_prior(user,repo)
...
#=====================================================
  ...
  def self.update_job_number(user,repo)
    ...
    All_repo_data_virtual.where("repo_name=? ","#{user}@#{repo}").find_all do |arry|
...
#====================================================
  def self.init_update_last_build_status#更新last_build_id
    @queue=SizedQueue.new(@thread_number)
    threads=[]
    @thread_number.times do 
      thread = Thread.new do
  ...
  @thread_number.times do 
    thread = Thread.new do
      loop do
        arry = @queue.deq
        break if arry == :END_OF_WORK
        info=Travistorrent.where("gh_project_name=? and tr_build_id=?",arry.repo_name.gsub('@','/'),arry.now_build_id).first
        # puts "commit"
        #   puts arry[0][:now_build_commit]
          if !info.nil?
          ...
          #arry[0].last_status=info[:status]
            ...
            info=Travistorrent_alldatas.where("gh_project_name=? and tr_build_id=?",arry.repo_name.gsub('@','/'),arry.now_build_id).first
...
def self.update_teamsize(user,repo)
  ...
  All_repo_data_virtual_prior_merge.where("repo_name=? and gh_team_size=0","#{user}@#{repo}").find_each do |info|
  ...
  thread = Thread.new do
    loop do
      ...
      to = repos.lookup(build[:last_build_commit])
        ...
        c=Diff_test.git_compare(build[:now_build_commit],build[:last_build_commit],info[1],info[2],rand(0..4))
        unless c.empty?
          puts "处理diff"
...
def self.update_timediff(user,repo)
  ...
  puts repo_name
  diff_arry=[]
  All_repo_data_virtual_prior_merge.find_by_sql("SELECT id,TIMESTAMPDIFF(HOUR,all_repo_data_virtual_prior_merges.now_start_at,all_repo_data_virtual_prior_merges.started_at) as duration FROM all_repo_data_virtual_prior_merges where time_diff is null").find_all do |info|
  #puts info
...
#====================================================
def self.merge_processdup(user,repo)#commit 和 branch都重复
  #c=Repo_data_travi.where( :repo_name => "#{user}@#{repo}").count
...
def self.update_prevchurn(user,repo)
  ...
  repo_name="#{user}@#{repo}"
  All_repo_data_virtual_prior_merge.where("repo_name=?",repo_name).order("id asc").find_all do |info|  
    #puts info  
  @queue.enq info
  end
  @thread_number.times do   
...
def init_queue_prevchurn
  @queue2=SizedQueue.new(@thread_number)
  threads=[]
  thread = Thread.new do
    loop do
      info = @queue.deq
  ...
  threads=[]
    ...
    thread = Thread.new do
      loop do
        ...
        All_repo_data_virtual_prior_merge.where("now_build_commit=?",info.commit).find_each do |all_repo|
        prev_srcchurn+=all_repo.src_churn
        prev_testchurn+=all_repo.test_churn
        end
        @queue2.enq [info,prev_srcchurn,prev_testchurn]
